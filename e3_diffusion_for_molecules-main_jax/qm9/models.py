...
class DistributionNodes:
    def __init__(self, histogram):
        ...
        prob = []
        self.keys = {}
        for i, nodes in enumerate(histogram): #iterate all nodes in histogram
            self.n_nodes.append(nodes) #count how many nodes there are in type i
            self.keys[nodes] = i # map number to nodes type index
            prob.append(histogram[nodes]) #record the frequency of the nodes type index i
    ...
    def sample(self, rng, n_samples=1):
        ...
        # idx = self.m.sample(sample_shape=(n_samples,), seed=rng)
        idx = jax.random.categorical(rng, self.prob, shape=(n_samples,))
...
class DistributionProperty:
    ...
    def _create_prob_dist(self, nodes_arr, values, distribution):
        ...
        for n_nodes in range(int(min_nodes), int(max_nodes) + 1):
            #Filter out all idx molecules with a n_nodes.
